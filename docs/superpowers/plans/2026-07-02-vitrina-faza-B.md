...
> **Статус: реализовано (2026-07-02), ветка `feature/vitrina-faza-B`.**
> ⚠️ В `master` ещё НЕ слито — стек PR ждёт мержа (`DEPLOY.md` §1).
> Коммиты: `9a72cae` (таблица `reviews`), `c1b7d06` (модуль `includes/reviews.php`),
> `f43d937` (отзывы на главной из БД), `e5ffbf4` (публичная форма отзыва),
> `e5314e0` (модерация в админке), `bd198fc` (кейсы отгрузок), `17c5e7c` (FAQ),
> `dde2dc7` (гарантии/документы), `bc46074` (FAB мессенджеров), `7c666c2` (регрессия).
> ⚠️ Иконки: план опирается на Phosphor (`ph-*`) — набор отменён коммитом
> `e9a2d48`, сайт переведён на FontAwesome.
